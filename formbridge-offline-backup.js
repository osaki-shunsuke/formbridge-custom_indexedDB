(function () {
    ...
    formBridge.events.on('form.show', async (context) => {
        ...
        try {
            ...
            // =========================================================================
            // 【設定】古いバックアップデータの自動クリーンアップ期限
            // 以下の数値を変更することで、何日経過で破棄するかを設定できます。（デフォルト: 7日間）
            // （※テストなどで短くしたい場合は「7」を「0」にして `+ 30 * 1000` などを足してください）
            // =========================================================================
            const EXPIRE_DAYS = 7;
            const EXPIRE_MS = EXPIRE_DAYS * 24 * 60 * 60 * 1000;

            // タイムスタンプが存在しない過去のデータも古すぎるとみなして破棄します
            if (backup && (!backup.__timestamp || Date.now() - backup.__timestamp > EXPIRE_MS)) {
                console.log(`🚮 古いバックアップ(${EXPIRE_DAYS}日以上経過)を自動破棄しました。`);
                await dbOp.clear();
                backup = null;
            }
